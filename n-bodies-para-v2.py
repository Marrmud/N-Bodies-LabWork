...
for step in range (NBSTEPS):
	...
	for i in range(rank*len(local_world), rank*len(local_world)+len(local_world)):
		#calculate all forces on body i
		for j in range(i):
			[dfx, dfy] = interaction(world[i],world[j])
			[fxi, fyi] = forces[i]
			forces[i] = [fxi+dfx, fyi+dfy]
			[fxj, fyj] = forces[j]
			forces[j] = [fxj-dfx, fyj-dfy]

	for i in range(0, len(forces)) :
		forces[i][0] = comm.allreduce(forces[i][0], op=MPI.SUM)
		forces[i][1] = comm.allreduce(forces[i][1], op=MPI.SUM)
