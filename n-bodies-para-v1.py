...
if(rank == 0) :
	print("Initializing Para Version")
...
for step in range (NBSTEPS):
	...
	local_world = comm.scatter(split(world,size),root=0)
	if rank == 0:
		forces = [[0,0] for _ in range(nbbodies)]
	forces = comm.bcast(forces,root=0)
	local_forces = comm.scatter(split(forces,size),root=0)
	for i in range(len(local_world)):
		#calculate all forces on body i
		for j in range(nbbodies):
			[fx, fy] = local_forces[i]
			[dfx, dfy] = interaction(local_world[i],world[j])
			local_forces[i] = [fx+dfx, fy+dfy]
	forces = comm.allgather(local_forces)
	if(rank == 0):
		forces = unsplit(forces)
...
exec_time = comm.reduce(end - start,op=MPI.MAX, root=0)

if (rank==0) : 
	print("Execution time for ",size,"processes: ",exec_time)
	print("Signature Seq = ", signature(world))		
